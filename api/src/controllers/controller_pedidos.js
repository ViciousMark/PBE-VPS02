...
const create = (req, res) => {
    const { idcliente, idpedido, idprod, idtelefone, datalancamento, total} = req.body;
    con.query('INSERT INTO projetocantina (idcliente, idpedido, idprod, idtelefone, datalancamento, total ) VALUES (?, ?)',
        [idcliente, idpedido, idprod, idtelefone, datalancamento, total],
        (err, result) => {
            if (err) {
                return res.status(500).json({ error: err.message });
            }
            return res.status(201).json({ id: result.insertId, idcliente, idpedido, idprod, idtelefone, datalancamento, total});
        });
}
const read = (req, res) => {
